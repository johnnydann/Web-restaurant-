...
public class MomoService : IMomoService
{
    ...
    public async Task<MomoCreatePaymentResponseModel> CreatePaymentAsync(OrderInfoModel model)
    {
        ...
        model.OrderInfo = "Khách hàng: " + model.FullName +  "Nội dung: Thanh toán MoMo cho Nhóm 05";
        var rawData =
    ...
    {
        // Sử dụng TryGetValue để kiểm tra sự tồn tại của khóa
        if (!collection.TryGetValue("amount", out var amount))
        {
            throw new InvalidOperationException("Key 'amount' not found");
        }

        if (!collection.TryGetValue("orderInfo", out var orderInfo))
        {
            throw new InvalidOperationException("Key 'orderInfo' not found");
        }

        if (!collection.TryGetValue("orderId", out var orderId))
        {
            throw new InvalidOperationException("Key 'orderId' not found");
        }
